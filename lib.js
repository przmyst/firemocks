...
// You should provide this JSON in your project (as you already do)
...
const DEFAULT_CHAIN_KEY =
	chainConfigData.defaultChainKey ||
	Object.keys(chainConfigData.chains || {})[0] ||
	'base-sepolia'

const CHAIN_CONFIGURATIONS = Object.freeze(
	Object.fromEntries(
		Object.entries(chainConfigData.chains || {}).map(([key, value]) => [
			key,
			Object.freeze({
				...value,
				nativeCurrency: value.nativeCurrency
					? Object.freeze({ ...value.nativeCurrency })
					: null
			})
		])
	)
...
const CHAIN_ALIAS_LOOKUP = (() => {
	...
	Object.entries(chainConfigData.aliases || {}).forEach(([alias, target]) => {
...
const DEFAULT_CHAIN_CONFIGURATION =
	CHAIN_CONFIGURATIONS[DEFAULT_CHAIN_KEY] ||
...
function findKnownChainByChainId(chainId) {
	...
	return (
		Object.values(CHAIN_CONFIGURATIONS).find(
			(entry) => entry.chainId === numeric
		) || null
	)
}
	...
	if (typeof candidate === 'object') {
		...
		if (candidate.rpcUrl) {
			...
			return {
				...
				explorerUrl:
					candidate.explorerUrl || candidate.blockExplorerUrl || candidate.blockExplorer || null,
				nativeCurrency: candidate.nativeCurrency ? { ...candidate.nativeCurrency } : null
			}
		}
		return null
	}
...
function resolveRpcOverride(preferences = {}, env = process.env, chainKey) {
	if (preferences.rpcUrl) return preferences.rpcUrl
	if (preferences.providerUrl) return preferences.providerUrl
	if (preferences.rpcEndpoint) return preferences.rpcEndpoint
	const source = env || process.env || {}
	if (source.RPC_URL) return source.RPC_URL
	if (chainKey) {
		const upper = String(chainKey).toUpperCase().replace(/[^A-Z0-9]+/g, '_')
		const candidates = [`${upper}_RPC_URL`, `${upper}_RPC_ENDPOINT`, `${upper}_RPC`]
...
function finalizeChainConfiguration(config, preferences = {}, env = process.env) {
	const base = cloneChainConfiguration(config)
	const overrideRpc = resolveRpcOverride(preferences, env, base?.key)
	if (overrideRpc) base.rpcUrl = overrideRpc
	return Object.freeze(base)
}

function resolveChainConfiguration(preferences = {}, env = process.env) {
	const attempts = []
	if (preferences.chainConfig) attempts.push(preferences.chainConfig)
	if (preferences.chainConfiguration) attempts.push(preferences.chainConfiguration)
	if (preferences.chain) attempts.push(preferences.chain)
	if (preferences.network) attempts.push(preferences.network)
	if (preferences.chainId != null) attempts.push(preferences.chainId)
	if (preferences.networkId != null) attempts.push(preferences.networkId)
	const source = env || process.env || {}
	const envCandidates = [
	...
	envCandidates.forEach((candidate) => {
		if (candidate != null && candidate !== '') attempts.push(candidate)
	})
	for (const candidate of attempts) {
		const config = coerceChainConfiguration(candidate)
		if (config) return finalizeChainConfiguration(config, preferences, source)
	}
	if (!DEFAULT_CHAIN_CONFIGURATION) throw new Error('No chain configurations available')
	return finalizeChainConfiguration(DEFAULT_CHAIN_CONFIGURATION, preferences, source)
}

/**
 * -----------------------------
 * Fireblocks-like enums
 * -----------------------------
 */
...
const TransactionSubStatus = Object.freeze({
	None: 'NONE'
})
...
function normalizePrivateKey(rawKey) {
	if (!rawKey) throw new Error('Fireblocks client requires a signing private key')
...
function parseAmount(value) {
	...
	if (/^0x[0-9a-fA-F]+$/.test(stringValue)) return BigInt(stringValue)
...
function initializeSigners(options = {}, sharedProvider) {
	...
	definitions.forEach((definition, index) => {
		...
		if (!byId.has(vaultId)) byId.set(vaultId, signer)
		byAddress.set(getAddress(signer.address), signer)
		const existingIndex = summaries.findIndex((entry) => entry.id === vaultId)
		if (existingIndex >= 0) {
			summaries[existingIndex] = { id: vaultId, address: signer.address }
		} else {
			summaries.push({ id: vaultId, address: signer.address })
		}
		if (!primary || definition.isPrimary) primary = signer
	})

	if (!primary) primary = byId.values().next().value
...
function collectSignerDefinitions(options = {}) {
	...
	function addDefinition(id, privateKey, isPrimary = false) {
		...
		definitions.push({ id: normalizedId, privateKey, isPrimary })
	}
...
function parseSignerDefinitions(value) {
		...
		if (Array.isArray(value)) {
			return value.flatMap((item, index) => {
				...
				if (typeof item === 'string') {
					return [{ id: String(index), privateKey: item }]
				}
				if (typeof item === 'object') {
					...
					if (privateKey) {
						return [{ id, privateKey, isPrimary: Boolean(item.isPrimary || item.primary) }]
					}
				}
				return []
			})
	...
	if (typeof value === 'object') {
		if (value.privateKey || value.secretKey || value.key) {
			...
			return privateKey
				? [{ id: id != null ? String(id) : undefined, privateKey, isPrimary: Boolean(value.isPrimary || value.primary) }]
				: []
		}
		...
		return Object.entries(value).flatMap(([id, item]) => {
			if (!item) return []
			if (typeof item === 'string') {
				return [{ id: String(id), privateKey: item }]
			}
			if (typeof item === 'object') {
				const privateKey = item.privateKey ?? item.key ?? item.secretKey ?? item.secret
				if (privateKey) {
					return [{ id: String(id), privateKey, isPrimary: Boolean(item.isPrimary || item.primary) }]
				}
			}
			return []
		})
	...
	if (tryJson) {
		if (Array.isArray(tryJson)) {
			...
			Object.entries(tryJson).forEach(([id, value]) => {
				if (!value) return
				if (typeof value === 'string') {
					entries.push({ id: String(id), privateKey: value })
...
function parseDelimitedSignerEntry(segment, index) {
	...
	if (delimiterMatch) {
		...
		if (!key) return null
		return { id: id || undefined, privateKey: key }
	}
	return { id: String(index), privateKey: segment }
}

function resolveProvider(options = {}, chainConfig) {
...
function normalizeOperation(rawOperation) {
	...
	if (normalized === TransactionOperation.ContractCall || normalized === 'CONTRACT_CALL') {
		return TransactionOperation.ContractCall
	}
	if (normalized === TransactionOperation.Raw || normalized === 'RAW') {
		return TransactionOperation.Raw
	}
	if (normalized === TransactionOperation.Transfer || normalized === 'TRANSFER') {
...
class Fireblocks {
	constructor(options = {}) {
		...
		const { primarySigner, signersById, signersByAddress, signerSummaries } =
		...
		this.transactions = {
			...
			cancelTransaction: (params) => this.cancelTransaction(params)
		}
	...
	async createTransaction(params = {}) {
		...
		const rawMessageData =
			transactionRequest?.extraParameters?.rawMessageData ||
		...
		if (operation === TransactionOperation.Raw || (rawMessageData && Array.isArray(rawMessageData.messages))) {
			if (!rawMessageData || !Array.isArray(rawMessageData.messages) || rawMessageData.messages.length === 0) {
				throw new Error('rawMessageData.messages is required and must be non-empty')
			}
			if (!rawMessageData.algorithm) {
				throw new Error('rawMessageData.algorithm is required (e.g., MPC_ECDSA_SECP256K1)')
			}
	...
	async cancelTransaction(params = {}) {
		...
		if ([TransactionStatus.Completed, TransactionStatus.Failed].includes(record.status)) {
			return { data: record }
		}
		record.status = TransactionStatus.Canceled
		record.subStatus = TransactionSubStatus.None
	...
	async _handleRawMessageTransaction(transactionRequest, rawMessageData = {}) {
		...
		// Lifecycle: QUEUED -> PENDING_SIGNATURE -> COMPLETED
		const record = {
			id: txId,
			status: TransactionStatus.Queued,
			subStatus: TransactionSubStatus.None,
			chain: this.chain,
			chainId: this.chainId,
			createdAt,
		...
		record.status = TransactionStatus.PendingSignature
		...
		for (const msg of messages) {
			...
			const publicKey = ensureHex(signer.signingKey.publicKey) // uncompressed

			signedMessages.push({
				content,
				algorithm: rawMessageData.algorithm,
				derivationPath: rawMessageData.derivationPath ?? transactionRequest.derivationPath ?? null,
				bip44AddressIndex: msg?.bip44AddressIndex ?? null,
				signature: {
		...
		record.signedMessages = signedMessages
	...
	async _handleContractCallTransaction(transactionRequest) {
		...
		if (!data) throw new Error('Contract call transaction requires calldata')
		...
		const txRequest = { to, data, value }
		if (gasLimit != null) txRequest.gasLimit = gasLimit
		if (gasPrice != null) {
			txRequest.gasPrice = gasPrice
		} else {
			if (maxFeePerGas != null) txRequest.maxFeePerGas = maxFeePerGas
			if (maxPriorityFeePerGas != null) txRequest.maxPriorityFeePerGas = maxPriorityFeePerGas
		}
	...
	async _handleTransferTransaction(transactionRequest) {
		...
		const txRequest = { to: destination, value }
		if (gasLimit != null) txRequest.gasLimit = gasLimit
		if (gasPrice != null) {
			txRequest.gasPrice = gasPrice
		} else {
	...
	_resolveContractCallData(transactionRequest) {
		if (transactionRequest?.extraParameters?.contractCallData) {
			...
			if (typeof raw === 'string') return ensureHex(raw)
			if (raw && typeof raw === 'object' && raw.calldata) return ensureHex(raw.calldata)
		}
		if (transactionRequest.contractCallData) {
			if (typeof transactionRequest.contractCallData === 'string') return ensureHex(transactionRequest.contractCallData)
			if (transactionRequest.contractCallData?.calldata) return ensureHex(transactionRequest.contractCallData.calldata)
		}
		if (transactionRequest.data) return ensureHex(transactionRequest.data)
		return null
	}
